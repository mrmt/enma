 ...
 *
 * $Id: foldstring.c 369 2008-08-15 14:24:57Z takahiko $
 */
/**
 * @file
 * @brief RFC2822 folding を考慮した文字列オブジェクト
 * @author takahiko@iij.ad.jp
 * @version $Id: foldstring.c 369 2008-08-15 14:24:57Z takahiko $
 */

#include "rcsid.h"
RCSID("$Id: foldstring.c 369 2008-08-15 14:24:57Z takahiko $");
 ...
 */
static int
FoldString_prefolding(FoldString *self, size_t size)
{
    if (0 != self->linepos && self->linelimits < self->linepos + size) {
        return FoldString_folding(self);
    }   // end if
    return 0;
}   // end function : FoldString_prefolding
...
FoldString_appendChar(FoldString *self, bool prefolding, char c)
{
    ...
    if (prefolding && 0 > FoldString_prefolding(self, 1)) {
        return -1;
    }   // end if
...
FoldString_appendBlock(FoldString *self, bool prefolding, const char *s)
{
    assert(NULL != self);
    size_t len = strlen(s);
    if (prefolding && 0 > FoldString_prefolding(self, len)) {
        return -1;
    }   // end if
...
/*
int
FoldString_appendInetMailbox(FoldString *self, bool prefolding, const InetMailbox *mailbox)
{
	assert(self && mailbox);
	size_t len = InetMailbox_getAddrSpecLength(mailbox);
	if (prefolding && 0 > FoldString_prefolding(self, len)) {
		return -1;
	}
	if (EOK != InetMailbox_writeMailbox(mailbox, self->xbuf)) {
		return -1;
	}
	self->linepos += len;

	return 0;
}	// end function : FoldString_appendInetMailbox
*/
