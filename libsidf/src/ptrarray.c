 ...
 *
 * $Id: ptrarray.c 41 2008-05-28 04:57:16Z takahiko $
 */
/**
 * @file
 * @brief void ポインタを格納する可変長配列
 * @author takahiko@iij.ad.jp
 * @version $Id: ptrarray.c 41 2008-05-28 04:57:16Z takahiko $
 */

#include "rcsid.h"
RCSID("$Id: ptrarray.c 41 2008-05-28 04:57:16Z takahiko $");
...
/**
 * PtrArray オブジェクトの解放
 * @param self 解放する PtrArray オブジェクト
 ...
 * @attention デストラクタを使用しておらず, 非 NULL の要素を上書きする場合はメモリリークに注意
 */
int
 ...
 * @return 成功した場合は新たに確保したメモリにおける配列の要素数, 失敗した場合は -1
 * @attention 確保する要素数は growth の倍数になるように調整される
 * @attention デストラクタを使用しておらず, 領域を縮小する場合はメモリリークに注意
 */
int
...
int
PtrArray_binarySearch(PtrArray *self, void *key,
...
int
PtrArray_linearSearch(PtrArray *self, void *key,
                      int (*kcompar) (const void *keyObj, const void *arrayElement))
{
    assert(NULL != self);
    assert(NULL != kcompar);
